...
pub async fn websocket_init(back_tx: Sender<Msg>, web_rx: Receiver<Msg>) {
    ...
    let chat = warp::path("chat")
        ...
        .map(|ws: warp::ws::Ws, users| {
            ...
            ws.on_upgrade(move |socket| user_connected(socket, users, back_tx))
        });
...
async fn user_connected(ws: WebSocket, users: Users, back_tx: Sender<Msg>) {
    ...
    tokio::task::spawn(rx.forward(back_tx).map(|result| {
